...
const LanguageMode: React.FC<Props> = (props: Props) => {
  ...
  const toggleLanguageMode = () => {
    languageMode === 'en' ? setLanguageMode('ru') : setLanguageMode('en')
  }

  return (
    <button onClick={toggleLanguageMode} className="group border border-solid border-[#2F2F2F] dark:border-[#E0E0E0] inline-flex rounded-full p-0.5 gap-0.5">
      <div className={`${props.size === 'small' ? 'p-1.5' : 'p-2'} ${languageMode === 'en' ? 'bg-[#2F2F2F] dark:bg-[#6d6d6d] group-hover:bg-[#343434]' : 'bg-transparent'} rounded-full flex justify-center items-center transition-colors`}>
      ...
      <div className={`${props.size === 'small' ? 'p-1.5' : 'p-2'} ${languageMode === 'ru' ? 'bg-[#2F2F2F] dark:bg-[#6d6d6d] group-hover:bg-[#343434]' : 'bg-transparent'} rounded-full flex justify-center items-center transition-colors`}>
        <i className={`${props.size === 'small' ? 'h-4 w-4' : 'h-5 w-5'}`}>
